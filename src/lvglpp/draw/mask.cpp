...
namespace lvgl::draw {
    ...
    LineMask::LineMask(lv_coord_t p1x, lv_coord_t p1y, lv_coord_t p2x,
        ...
        this->lv_obj = LvPointerType(static_cast<LvClass*>(lv_malloc(sizeof(LvClass))));
        lv_draw_sw_mask_line_points_init(this->lv_obj.get(), p1x, p1y, p2x, p2y, side);
    }

    LineMask::LineMask(lv_coord_t px, lv_coord_t py, int16_t angle,
                       lv_draw_sw_mask_line_side_t side) {
        this->lv_obj = LvPointerType(static_cast<LvClass*>(lv_malloc(sizeof(LvClass))));
        lv_draw_sw_mask_line_angle_init(this->lv_obj.get(), px, py, angle, side);
    }
    ...
    AngleMask::AngleMask(lv_coord_t vertex_x, lv_coord_t vertex_y,
        ...
        this->lv_obj = LvPointerType(static_cast<LvClass*>(lv_malloc(sizeof(LvClass))));
        lv_draw_sw_mask_angle_init(this->lv_obj.get(), vertex_x, vertex_y, start_angle, end_angle);
    }
    

    RadiusMask::RadiusMask(const misc::Area & rect, lv_coord_t radius, bool inv) {
        this->lv_obj = LvPointerType(static_cast<LvClass*>(lv_malloc(sizeof(LvClass))));
        lv_draw_sw_mask_radius_init(this->lv_obj.get(), rect.raw_ptr(), radius, inv);
    }

    RadiusMask::RadiusMask(const lv_area_t & rect, lv_coord_t radius, bool inv) {
        this->lv_obj = LvPointerType(static_cast<LvClass*>(lv_malloc(sizeof(LvClass))));
        lv_draw_sw_mask_radius_init(this->lv_obj.get(), &rect, radius, inv);
    }
    ...
    FadeMask::FadeMask(const misc::Area & coords, lv_opa_t opa_top, lv_coord_t y_top,
        ...
        this->lv_obj = LvPointerType(static_cast<LvClass*>(lv_malloc(sizeof(LvClass))));
        lv_draw_sw_mask_fade_init(this->lv_obj.get(), coords.raw_ptr(), opa_top, y_top, opa_bottom, y_bottom);
    }

    FadeMask::FadeMask(const lv_area_t & coords, lv_opa_t opa_top, lv_coord_t y_top,
                       lv_opa_t opa_bottom, lv_coord_t y_bottom) {
        this->lv_obj = LvPointerType(static_cast<LvClass*>(lv_malloc(sizeof(LvClass))));
        lv_draw_sw_mask_fade_init(this->lv_obj.get(), &coords, opa_top, y_top, opa_bottom, y_bottom);
    }


    MapMask::MapMask(const misc::Area & coords, const std::vector<lv_opa_t> & map) {
        this->lv_obj = LvPointerType(static_cast<LvClass*>(lv_malloc(sizeof(LvClass))));
        lv_draw_sw_mask_map_init(this->lv_obj.get(), coords.raw_ptr(), map.data());
    }

    MapMask::MapMask(const lv_area_t & coords, const std::vector<lv_opa_t> & map) {
        this->lv_obj = LvPointerType(static_cast<LvClass*>(lv_malloc(sizeof(LvClass))));
        lv_draw_sw_mask_map_init(this->lv_obj.get(), &coords, map.data());
    }
}
